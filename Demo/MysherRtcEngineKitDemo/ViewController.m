...
@interface ViewController () <MysherRtcEngineDelegate>
...
- (void)viewDidLoad {
    ...
    _kit = [MysherRtcEngineKit sharedEngineWithDelegate:self];
}
...
- (BOOL)rtcEngine:(MysherRtcEngineKit * _Nonnull)engine didLoginStatus:(MysherLoginStatusCode)status
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if(status == MYSHER_LOGIN_SUCCESS)
        {
        ...
        else if(status == MYSHER_LOGIN_ERROR_NETWORK)
        {
            self.text.text = @"没有网络";
        }
        else if(status == MYSHER_LOGIN_ERROR_CONFLICT)
        {
            self.text.text = @"异地登陆";
        }
        else if(status == MYSHER_LOGIN_ERROR_UNAUTHORIZED)
        {
            self.text.text = @"密码错误";
        }
        else
